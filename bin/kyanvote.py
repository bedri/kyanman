...
git_dir = os.path.abspath(
    os.path.join(
        ...
        '..'))
kyan_conf_dir = os.path.join(os.getenv('HOME'), '.kyancore')
kyan_cli_path = os.getenv('KYAN_CLI')
if os.getenv('KYANMAN_PID') is None:
    quit("--> please run using 'kyanman vote'")
...
def run_kyan_cli_command(cmd):
    return run_command("%s %s" % (kyan_cli_path or 'kyan-cli', cmd))
...
def submit_votes(win, ballot, s):
    ...
    if votes_to_send.keys():
        ...
        for mn in shuffled_mn_keys:
            ...
            if background_send:
                deferred_votes = tempfile.NamedTemporaryFile(prefix=('sending_kyanvote_votes-%s=' % batch_timestamp),delete=False)
            ...
            for vote in shuffled_vote_keys:
                ...
                signature = kyanutil.sign_vote(netvote, mnprivkey)
                command = ('%s' % kyan_cli_path is not None and kyan_cli_path or 'kyan-cli') + ' voteraw ' + str(node['txid']) + ' ' + str(node['txout']) + ' ' + str(
        ...
        if background_send:
            voter_parent = tempfile.NamedTemporaryFile(prefix=('sending_kyan_votes-%s-' % batch_timestamp), delete=False)
...
def main(screen):
    ...
    if kyan_cli_path is None:
        # test kyan-cli in path -- TODO make robust
        try:
            run_command('kyan-cli getinfo')
        except subprocess.CalledProcessError:
            quit(
                "--> cannot find kyan-cli in $PATH\n" +
                "    do: export PATH=/path/to/kyan-cli-folder:$PATH\n" +
                "    and try again\n")

    loadwin = curses.newwin(40, 40, 1, 2)

    loadwin.addstr(1, 2, 'kyanvote version: ' + version, C_CYAN)

    mncount = int(run_kyan_cli_command('masternode count enabled'))
    block_height = int(run_kyan_cli_command('getblockcount'))
    ...
    # get ballot
    ballots = json.loads(run_kyan_cli_command('gobject list all'))
    ballot = {}
    ...
    for entry in ballots:
        ...
        ballots[entry][u'votes'] = json.loads(run_kyan_cli_command('gobject getvotes %s' % entry))
    ...
    masternodes = {}
    with open(os.path.join(kyan_conf_dir, 'masternode.conf'), 'r') as f:
        lines = list(
            line
            for line in
            (l.strip() for l in f)
        ...
        for line in lines:
            ...
            masternodes[conf[3] + '-' + conf[4]] = {
                ...
                "txout": conf[4]}
    if not masternodes:
        # fallback to kyan.conf entries if no masternode.conf entries
        with open(os.path.join(kyan_conf_dir, 'kyan.conf'), 'r') as f:
            lines = list(
                line
                for line in
                (l.strip() for l in f)
            ...
            if all(k in conf for k in ('masternode', 'externalip', 'masternodeprivkey')):
                # get funding tx from kyanninja
                import urllib2
                mninfo = urllib2.urlopen(
                    "https://kyanninja.pl/api/masternodes?ips=[\"" +
                ...
                try:
                    ...
                    quit('cannot retrieve masternode info from kyanninja')
                ...
                quit('cannot find masternode information in kyan.conf')
    ...
    votewin.addstr(1, 2, 'kyanvote version: ' + version, C_CYAN)
    votewin.addstr(
        2,
        2,
